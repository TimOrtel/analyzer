...
def cummulative_distr_compare2(outdir, result_csv_filename):
    ...
    data, base = utils.create_cum_data(df, num_bins, [utils.header_runtime_parent, utils.header_runtime_everything_disabled])
    ...
    data, base = utils.create_cum_data(df, num_bins, [utils.header_runtime_parent, utils.header_runtime_locals_enabled])
...
def cummulative_distr_all3(outdir, result_csv_filename):
    ...
    data, base = utils.create_cum_data(df, num_bins, [utils.header_runtime_parent, utils.header_runtime_everything_disabled, utils.header_runtime_locals_enabled])
    datanonincr = {"values": data[0], "label": "Non-incremental analysis of parent commit"}
    dataincr = {"values": data[1], "label": "Incremental analysis of commit"}
...
def distribution_absdiff_plot(title, result_csv_filename, outdir, cutoffs_incr=None, cutoffs_rel=None):
    ...
    diff = df.loc[:,utils.header_runtime_parent] - df.loc[:,utils.header_runtime_everything_disabled]
    ...
    diff = df.loc[:,utils.header_runtime_everything_disabled] - df.loc[:, utils.header_runtime_locals_enabled]
...
def distribution_reldiff_plot(title, result_csv_filename, outdir, cutoffs_incr=None, cutoffs_rel=None):
    ...
    # plot incremental vs non-incremental
    print(df[utils.header_runtime_everything_disabled].astype('float'))
    diff = 1 - df[utils.header_runtime_everything_disabled].astype('float') / df[utils.header_runtime_parent].astype('float')
    ...
    # plot reluctant vs. basic incremental
    diff = 1 - df.loc[:,utils.header_runtime_locals_enabled] / df.loc[:, utils.header_runtime_everything_disabled]
...
def paper_efficiency_graphs(dir_results, csv_filename, outdir, filterRelCLOC=False, filterDetectedChanges=False):
    ...
    diff1 = 1 - df[utils.header_runtime_everything_disabled].astype('float') / df[utils.header_runtime_parent].astype('float')
    diff2 = 1 - df[utils.header_runtime_locals_enabled].astype('float') / df[utils.header_runtime_everything_disabled].astype('float')
    diff3 = 1 - df[utils.header_runtime_everything_disabled].astype('float') / df[utils.header_runtime_locals_enabled].astype('float')
    diff4 = 1 - df[utils.header_runtime_locals_enabled].astype('float') / df[utils.header_runtime_parent].astype('float')
    step = 0.01
    for i, diff in enumerate([diff1,diff2,diff3,diff4]):
        ...
        outfile = os.path.join(outdir, "efficiency_figure_" + str(i) + ".png")
        if i == 0:
    ...
    diff1 = df[utils.header_runtime_everything_disabled].astype('float') / df[utils.header_runtime_parent].astype('float')
    diff2 = df[utils.header_runtime_locals_enabled].astype('float') / df[utils.header_runtime_everything_disabled].astype('float')
    diff3 = df[utils.header_runtime_locals_enabled].astype('float') / df[utils.header_runtime_parent].astype('float')
